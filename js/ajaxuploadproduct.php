	...
	function uploadImage($fileName, $maxSize, $maxW, $fullPath, $relPath, $colorR, $colorG, $colorB, $maxH = null){
		...
		if($filesize > 0){	
			...
			if($filesize > $maxlimit){ 
				$errorList[] = "The file size limit is 1 MB.";
			}
		...
		if(!$match){
		   	$errorList[]= " File type is not allowed. Please upload only image file.";
		}
...
?>
<?php //$localhost_images = Yii::app()->images->baseUrl;// localhost solution
	if($imgUploaded){
	
	// Define Host
	if($_SERVER['HTTP_HOST'] != "luckydshop.com" and $_SERVER['HTTP_HOST'] != "www.luckydshop.com"){$host = $_SERVER['HTTP_HOST'].'/luckd';}
	else {$host = $_SERVER['HTTP_HOST'];}
	$fname = substr("$upload_image", 17);	// Cut Image path name
	
	// Resize Stoge Logo		
	echo '<div style="position:absolute"><img src="http://'.$host.'/js/imageresizestorelogo.php/'.$fname.'?name='.$fname.'&image=http://'.$host.'/'.$upload_image.'" /></div>';
	echo '<div style="position:relative"><img src="http://'.$host.'/'.$upload_image.'" width="200px" border="0" /></div>';
		
echo '
	<script>
		parent.document.getElementById("Product_product_photo").value="'.$upload_image.'"';
